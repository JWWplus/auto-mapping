...
def getinfo_by_id(id_number):
    ...
    # 更新单条数据,并将修前后的数据写入到log文件中
        ...
        update_record.note = data.get('note', '')
    ...
    if request.method == 'GET':
        ...
        if data:
            ...
            data_insert = PageInfo(data['page'], data['event'], data['object'], appver,
                                   ...
                                   data['se_action'], data.get('note', ''), data['platform'],
    ...
    if request.method == 'POST':
        ...
        if not page:
            if AppVersion:
                ...
                page_type = db.session.query(PageInfo.page, func.count(PageInfo.page).label('sum')).filter_by(
                    appversion_id=appversion.id).group_by(PageInfo.page).order_by('sum desc')
                if platform:
                    if event:
                ...
                page_type = db.session.query(PageInfo.page, func.count(PageInfo.page).label('sum')).filter_by(
                    platform=platform).group_by(PageInfo.page).order_by('sum desc')
                ...
                resp = PageInfo.query.order_by('object asc').limit(
                    numPerPage).offset((currentPage - 1) * numPerPage)
        ...
        Datainfo = {
            ...
            'count': count,
            'page_type': page_type.all() if page_type else page_type,
        }
